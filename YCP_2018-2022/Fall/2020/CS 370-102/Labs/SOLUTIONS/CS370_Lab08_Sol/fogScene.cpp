...
vec4 fogColor = {0.2f, 0.2f, 0.2f, 1.0f};
float fogStart = 1.0f;
float fogEnd = 8.0f;
...
GLuint cam_mat_loc;
GLuint fog_col_loc;
GLuint fog_start_loc;
GLuint fog_end_loc;
const char *vertex_shader = "../color_mesh.vert";
const char *frag_shader = "../fog.frag";
...
int main(int argc, char**argv)
{
	...
	GLFWwindow* window = CreateWindow("Fog Scene");
    if (!window) {
    ...
    fog_col_loc = glGetUniformLocation(program, "fogColor");
    fog_start_loc = glGetUniformLocation(program, "fogStart");
    fog_end_loc = glGetUniformLocation(program, "fogEnd");
...
//    mat4 camera_matrix = mat4().identity();
    ...
    proj_matrix = frustum(-1.0f*xratio, 1.0f*xratio, -1.0f*yratio, 1.0f*yratio, 1.0f, 20.0f);
...
void render_scene( ) {
    ...
    glUniform4fv(fog_col_loc, 1, fogColor);
    glUniform1f(fog_start_loc, fogStart);
    glUniform1f(fog_end_loc, fogEnd);
